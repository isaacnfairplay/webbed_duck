#!/usr/bin/env python3
"""Update the auto-generated complexity history markdown.
...
for humans (including a Mermaid xychart).
"""
...
def render_mermaid(entries: Sequence[Entry]) -> str:
    lines = ["```mermaid", "xyChart", "    title: Webbed Duck Cyclomatic Complexity", "    xType: category", "    xLabel: Version", "    yLabel: Average Cyclomatic Complexity"]
    lines.append("    series:")
    lines.append("      - title: Average CC")
    lines.append("        data:")
    for entry in entries:
        lines.append(f"          - x: v{entry.version}")
        lines.append(f"            y: {entry.average:.3f}")
    lines.append("```")
    return "\n".join(lines)
...
def rebuild_markdown(entries: Sequence[Entry]) -> str:
    header = (
    ...
    return "\n\n".join([header, mermaid, data_block, *body_sections]) + "\n"
...
def main() -> None:
    ...
    markdown = rebuild_markdown(ordered)
