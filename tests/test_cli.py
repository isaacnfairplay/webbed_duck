...
def test_watch_iteration_detects_constant_change(tmp_path: Path) -> None:
    source_dir = tmp_path / "src"
    build_dir = tmp_path / "build"
    source_dir.mkdir()
    build_dir.mkdir()

    toml_path = source_dir / "sample.toml"
    sql_path = source_dir / "sample.sql"
    ...
    snapshot = cli.build_source_fingerprint(source_dir)

    calls: dict[str, int] = {}

    def fake_compile_and_reload(*_args, **_kwargs) -> int:  # type: ignore[no-untyped-def]
    ...
    updated, reloaded = cli._watch_iteration(
        object(),
        source_dir,
        build_dir,
        snapshot,
        compile_and_reload=fake_compile_and_reload,
    )
