...
def _render_markdown(routes_compiled: int, records: list[dict[str, object]], *, generated_at: datetime) -> str:
    ...
    lines.append("# Preprocess Module Reference Demo")
    lines.append("")
    ...
    lines.append("```")
    lines.append("")
    lines.append("The `greeting` and `generated_at` fields are injected by the module-backed preprocessor.")
    lines.append("")
    return "\n".join(lines)
