    ...
    *,
    ...
    for step in steps:
        try:
            ...
            raise RuntimeError(f"Preprocess step is misconfigured: {exc}") from exc
        ...
        if not isinstance(updated, Mapping):
            raise TypeError(
                f"Preprocessor '{reference.describe()}' must return a mapping or None, received {type(updated)!r}"
            )
...
def _load_callable(reference: CallableReference) -> Callable[..., Mapping[str, Any] | None]:
    cache_key = reference.cache_key
    if cache_key in _CACHE:
        return _CACHE[cache_key]
    target = load_callable(reference)
