...
def _ensure_dir_writable(path: Path) -> None:
    """Ensure ``path`` exists and is writable by creating and deleting a probe."""
...
def _load_toml(path: Path) -> Mapping[str, Any]:
    if not path.exists():
        raise ConfigError(
            f"Configuration file not found: {path}. Provide a config.toml with [runtime].storage."
        )
...
def load_config(path: str | Path | None = None) -> Config:
    ...
    try:
        _ensure_dir_writable(storage_path)
